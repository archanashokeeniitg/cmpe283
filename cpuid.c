...
// Results cmpe-283 Ass 2- Archana
/* 
Part- For CPUID leaf node %eax=0x4FFFFFFF:
*/
...
if (eax == 0x4FFFFFFF) {
		long time = atomic_long_read(&times);
		// Return the total number of exits (all types) in %eax
		eax = atomic_read(&exit_Num);
...
/*
Part- For CPUID leaf node %eax=0x4FFFFFFE:
*/
...
int kvm_emulate_cpuid_2(struct kvm_vcpu *vcpu)
{
	...
	if (eax == 0x4FFFFFFE) {
        ...
        // return the  CPUID(0x4FFFFFFF) Return the total number of exits (all types)
